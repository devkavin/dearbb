...
export default function RevealPage() {
  ...
  const prefilledMessage = useMemo(() => `Hey love, I chose: ${optimisticChoice ?? 'still deciding'}${note ? ` | note: ${note}` : ''}.`, [optimisticChoice, note]);
  ...
  const downloadDateCard = () => {
    ...
    const svg = `<svg xmlns='http://www.w3.org/2000/svg' width='640' height='360'><rect width='100%' height='100%' fill='#f7eaff'/><rect x='28' y='28' width='584' height='304' rx='18' fill='#ffffff' stroke='#b9a2ee' stroke-width='4'/><text x='52' y='90' font-size='34' fill='#6a4ca5'>Dear bb</text><text x='52' y='145' font-size='22' fill='#3f3856'>Selection: ${optimisticChoice ?? 'pending'}</text><text x='52' y='185' font-size='20' fill='#4f4a63'>Plan: ${activity} at ${time}</text><text x='52' y='225' font-size='18' fill='#4f4a63'>${note || 'No extra note yet'}</text><text x='52' y='280' font-size='20'>💖 ⭐ 📖 🎶</text><text x='52' y='315' font-size='14' fill='#64607a'>${stamp}</text></svg>`;
